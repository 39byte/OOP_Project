...
class MenuItem(ABC):
    ...
    def get_price(self): return self.price      # 가격 반환

    @abstractmethod
    def get_recipe(self): pass                  # 레시피 반환
...
class Customer(Base):   # 손님들 공통 클래스
    ...
    @abstractmethod
    def order(self, menu_list): pass    # 주문 받기 기능
    @abstractmethod
    def pay(self, total_price): pass    # 정산하기 기능
...
class NormalCustomer(Customer):
    ...
    def pay(self, total_price): return total_price      # 정가 지불
...
class GrillStation:
    ...
    def get_click_result(self, stock_manager):
        ...
        upper_bount = (self.cook_time + self.precision_window / 2)      # 5.3초

        if  lower_bound <= self.timer <= upper_bount:       # 성공시 조리된 패티 추가
